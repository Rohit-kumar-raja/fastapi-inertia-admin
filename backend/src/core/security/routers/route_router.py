...
async def edit(uuid: UUID, session: AsyncSession = Depends(get_db)):
    ...
    data = await PermissionService.get_by_id(uuid, session=session)
    if data is None:
        return error_response(message="Data not found", status_code=404)
    return response(data=data, message="Data fetched successfully")
...
async def create(permission: RouteSchema, session: AsyncSession = Depends(get_db)):
    ...
    is_unique = await PermissionService.is_unique(name=permission.name, parent_id=permission.parent_id, session=session)
    if is_unique:
        return error_response(message="Permission name already exists ", status_code=422)
    response_data = await PermissionService.create(permission.model_dump(), session=session)
...
async def update(role: RouteSchema, uuid: UUID, session: AsyncSession = Depends(get_db)):
    ...
    data = await PermissionService.update(uuid, role.model_dump(), session=session)
...
async def destroy(uuid: UUID, session: AsyncSession = Depends(get_db)):
    ...
    permision = await PermissionService.get_by_id(uuid, session=session)
    if permision is None:
        return error_response(message="Data not found", status_code=404)
    if permision.children != []:
        return error_response(message="You can't delete the parent Route", status_code=400)

    data = await PermissionService.delete(uuid, session=session)
...
async def get_permission_tree(session: AsyncSession = Depends(get_db)):
    data = await PermissionService.get_permission_tree(session=session)
    if data is None:
